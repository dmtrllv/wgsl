import { Position, spanWithSize } from "@wgsl/core";
import { isIdentifier, isKeyword, isOperator, isSeparator, isWhitespace, token } from "./token.js";
import { Iter } from "./iter.js";
export const parseSource = (path, source, ctx) => {
    return ctx.try(() => {
...
const parseToken = (type, str, cursor) => {
    if (type === "Unknown")
...
const parseCharLiteral = (iter, cursor) => {
    let str = iter.next();
    let escape = false;
    str += iter.collectWhile(c => {
        if (escape) {
...
const parseStrLiteral = (iter, cursor) => {
    let str = iter.next();
    let escape = false;
    str += iter.collectWhile(c => {
        if (escape) {
